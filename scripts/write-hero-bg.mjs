...
export function HeroBackground({
  ...
  return (
    <>
      ${o} className="absolute inset-0 z-0 overflow-hidden" aria-hidden>
        <motion.div
          style={parallaxMotionStyle}
          className={cn("absolute inset-0", useParallax && "will-change-transform")}
        >
          ${o} className={cn("absolute inset-0", mediaScaleClass)} style={{ transformOrigin: "center center" }}>
            ${o} className="absolute inset-0" style={{ filter: mediaFilter }}>
              <CmsImage
                src={imageSrc}
                alt={imageAlt}
                fill
        ...
        {videoUrl && allowVideo ? (
          ${o} className="absolute inset-0 z-[1] overflow-hidden">
            ${o} className={cn("absolute inset-0", mediaScaleClass)} style={{ transformOrigin: "center center" }}>
              <video
                key={videoUrl}
      ...
      ${o} className="pointer-events-none absolute inset-0 z-10" aria-hidden>
        ${o}
        ...
        {cinematic ? (
          <>
            ${o} className="absolute inset-y-0 left-0 w-[min(72%,42rem)] bg-gradient-to-r from-black/42 via-black/18 to-transparent" />
            ${o} className="absolute inset-y-0 right-0 w-1/3 bg-gradient-to-l from-slate-900/16 to-transparent" />
          </>
        ) : null}
        ...
        ${o} className="absolute inset-x-0 bottom-0 h-[42%] bg-gradient-to-t from-background/88 via-background/28 to-transparent dark:from-background/82 dark:via-background/24" />

        ${o} className="noise-overlay absolute inset-0 [opacity:0.09] dark:[opacity:0.12]" />

        {showVignette ? (
          ${o} className="absolute inset-0 bg-[radial-gradient(ellipse_90%_70%_at_50%_50%,transparent_48%,rgba(0,0,0,0.28)_100%)]" />
        ) : null}
      ${c}

      ${o} className="pointer-events-none absolute inset-0 z-20" aria-hidden>
        {cinematic ? (
          <>
            ${o} className="absolute inset-0 bg-[radial-gradient(ellipse_80%_55%_at_50%_-5%,rgba(56,189,248,0.18),transparent_62%)] animate-glow-pulse" />
            ${o} className="absolute inset-0 bg-[radial-gradient(ellipse_50%_40%_at_12%_50%,rgba(0,0,0,0.22),transparent_70%)]" />
          </>
        ) : null}

        {showFog ? (
          ${o} className="absolute inset-0 animate-hero-fog bg-gradient-to-tr from-primary/5 via-transparent to-accent/6 mix-blend-soft-light dark:from-primary/8 dark:to-accent/10" />
        ) : null}

        {showAirflow ? <HvacAirflow className="absolute inset-0 h-full w-full" intensity={0.75} /> : null}

        {showParticles ? (
          ${o} className="absolute inset-0 overflow-hidden">
        ...
        ${o} className="absolute inset-0 [background-image:linear-gradient(to_right,rgba(255,255,255,0.04)_1px,transparent_1px),linear-gradient(to_bottom,rgba(255,255,255,0.04)_1px,transparent_1px)] [background-size:44px_44px]" />
      ${c}
    </>
  );
}
...
fs.writeFileSync("components/sections/hero-background.tsx", content);
console.log("wrote hero-background.tsx, close tag:", c);
